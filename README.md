...
ユーザーが意見を投稿
    ↓
意見解析LLMが評価軸ごとの事前パラメータを推定
    ↓
ベイズ投票エンジンが猫ごとの投票値を決定（-10〜+10）
    ↓
...
- 意見投稿 → **即時初回投票**（意見解析LLMは投稿時に1回、投票値はベイズエンジンで決定）
...
- 投票後に猫たちの反応から意見パラメータの事後分布を更新
- 確定した投票値・判断要因をもとにコメント生成LLMが猫の発言を生成
...
| `OPENROUTER_MODEL` | - | 旧設定との互換用。意見解析・コメントモデルの共通フォールバック |
| `OPINION_INTERPRETER_MODEL` | - | 意見本文を評価軸の値へ変換するモデル（既定 `google/gemini-2.5-flash`） |
| `VOTE_COMMENT_MODEL` | - | 確定済み投票値からコメントを生成するモデル（既定 `openai/gpt-4o-mini`） |
...
│  ├─ llm.ts                      # 意見解析LLM・コメントLLM・フォールバック
│  ├─ bayes.ts                    # ベイズ投票・意見パラメータ事後推定
...
- **LLMが投票値を決めない**: 意見解析LLMは投稿時の評価軸パラメータ推定だけを行う。投票値・権力変動・派閥成立/解散/破門・Point計算・決選投票はベイズ／Rule Engineが決定論的に処理する。コメントLLMは確定済みの投票値を文章化するだけ。
- **意見パラメータの保存**: `parameter_prior` と `parameter_posterior` を分け、投稿時の初期推定を上書きせずに投票反応を次回以降へ反映する。
